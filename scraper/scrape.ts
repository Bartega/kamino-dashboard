...
import { scrapeLend } from "./scrapers/lend.js";
...
async function main() {
  ...
  try {
    ...
    if (!pageFilter || pageFilter === "lend") {
      console.log("[scraper] Scraping lend page...");
      const result = await withRetry(() => scrapeLend(page));
      data.lend = result.vaults;
      data.stats.lendMarketSize = result.marketSize;
      data.stats.lendVaultDeposits = result.vaultDeposits;
    }
